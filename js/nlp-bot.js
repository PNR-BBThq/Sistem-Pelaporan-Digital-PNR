...
// =======================================================================
// FUNGSI UI: DRAG, RESIZE, BUTTON, DAN WINDOW CHAT
// =======================================================================
...
(function() {
    ...
    // Kalau DOM dah siap, terus init
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        setTimeout(initChatUI, 100);
    }

    function initChatUI() {
        const chatWindow = document.getElementById('nlp-chatbot-window');
        const chatHeader = document.getElementById('nlp-chatbot-header');
        const chatInput = document.getElementById('nlp-chat-input');
        const chatSendBtn = document.getElementById('nlp-chat-send-btn');
        const chatToggleBtn = document.getElementById('nlp-chatbot-toggle-btn');
        ...
        if (chatWindow && chatHeader) {
            ...
            document.addEventListener('mousemove', function(e) {
                if (isDragging) {
                    ...
                    // Hadkan supaya tak keluar skrin
                    newX = Math.max(0, Math.min(newX, window.innerWidth - chatWindow.offsetWidth));
        ...
        // ========== 3. FUNGSI SEND BUTTON ==========
        if (chatSendBtn) {
            chatSendBtn.addEventListener('click', function() {
                SmartNLPBot.handleInput();
            });
        }
        ...
        // ========== 5. FUNGSI CHIP CLICK (KALAU ADA) ==========
        document.querySelectorAll('.nlp-chip').forEach(chip => {
            chip.addEventListener('click', function() {
                SmartNLPBot.handleInput(this.textContent.trim());
            });
        });

        // ========== 6. FUNGSI CLOSE BUTTON (KALAU ADA) ==========
        const closeBtn = document.getElementById('nlp-chatbot-close-btn');
        if (closeBtn) {
        ...
        // ========== 7. FUNGSI MINIMIZE BUTTON (KALAU ADA) ==========
        const minimizeBtn = document.getElementById('nlp-chatbot-minimize-btn');
        if (minimizeBtn) {
